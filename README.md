...
- 不识别型号、规格、货号、数量、价格。
- 触发词同级：`LOT`、`Lot No`、`Lot Number`、`Batch`、`S/N`、`Serial No`、`Retrace Code`、`厂商批号`、`产品批号`、`批号`、`生产批号`、`批次号`、`批次代码`、`序列号`、`产品序列号`、`序号`、`出厂编号`。
- 字段名本身不能作为候选值；例如 `序列号 Serial no: 70552605046` 只返回 `70552605046`，不能把 `Serial`、`Serial no`、`No`、`序列号` 写入候选。
- `产品批号`后的值允许清理连接符两侧空格，例如 `26197 - 0324` 归一化为 `26197-0324`；型号、规格和产品名称不能作为批号。
- 本阶段不做条形码解码；条形码图形旁没有明文编号时，不从条形码图形推断批号。
- 日期字段必须有明确字段、图标或日期语义才返回；生产日期只到年月时补为当月 1 号，失效日期只到年月时补为当月最后一天，例如失效日期 `02/2027` 返回 `2027-02-28`、闰年 `02/2028` 返回 `2028-02-29`；`092127`、`260307` 等 6 位日期只在格式不歧义时归一化。
- 普通产品标签中的 `Date/DATE` 只有位于 `Batch/LOT`、`Shelf Life/有效期` 等生产信息区域，且不存在打印、包装、出库、失效等相反语义时，才按生产日期处理；日期格式不固定，明确的时间后缀会被忽略，年月日顺序有歧义时留空。
- 普通产品标签同时出现 `Date/DATE`、`Batch/LOT` 和 `Shelf Life/有效期` 时，`Date/DATE` 按生产日期处理，`Shelf Life/有效期` 是从该日期起算的有效时长，不能反向把 `Date/DATE` 当成失效日期。
- 根据明确生产日期和 `Shelf Life/有效期` 推算失效日期时按日历年或月直接相加，不减一天。
- 明确看到表格表头“厂商批号”时按出库单处理：从该列任选一条最清晰的非空数据行，只返回这一行的厂商批号和同一行到期日期；不返回货号或其他单据字段。
...
- 多候选且存在明确强弱时，`batch_number` 和 `candidates[0]` 均为模型选出的高概率候选；证据并列时 `batch_number` 留空，继续人工确认
- 日期包含字母、非法字符、非法月日或无法唯一解析时直接置空，不自动替换易混淆字符
- 生产日期晚于 `Asia/Shanghai` 当天时直接置空；未来失效日期不受此规则影响
- 出库单多行时只保留选中的一行，`batch_number` 与 `candidates[0]` 相同；无法清晰识别批号时，对应日期也留空
...
- `ai_timeout_ms`：模型调用超时阈值。默认 10000ms；超过后接口中止 AI 请求，按 `status=not_found` 返回空批号，继续进入人工确认/填写。
